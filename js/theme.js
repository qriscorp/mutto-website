    ...
    const observer = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        if (entry.intersectionRatio > 0) {
          ...
          const pluginOptions = theme.fn.getOptions(
...
((theme = {}, $) => {
  ...
  if ($(".thumb-info-floating-element-wrapper").length) {
    if (typeof gsap !== "undefined") {
      $(".thumb-info-floating-element-wrapper")
        .on("mouseenter", function ({ clientX, clientY }) {
          if (!$(this).data("offset")) {
            $(this).data("offset", 0);
          }
  ...
  if ($(".style-switcher-open-loader").length) {
    ...
    $(".style-switcher-open-loader").on("click", function (e) {
      ...
      const basePath = $(this).data("base-path"),
  ...
  class PluginAnimate {
    ...
    build() {
      ...
      if (self.options.flagClassOnly) {
        const delay = self.options.wrapper.attr("data-appear-animation-delay")
  ...
  class PluginCarousel {
    ...
    build() {
      ...
      if ($el.attr("data-sync")) {
        $el.on(
          "change.owl.carousel",
          ({ namespace, property, relatedTarget }) => {
  ...
  class PluginCounter {
    ...
    build() {
      ...
      if (self.options.comma) {
        self.options.formatter = (value, { decimals }) =>
  ...
  class PluginGDPRWrapper {
    ...
    build() {
      ...
      if (
        ...
        $.cookie("porto-gdpr-preferences").includes(self.options.checkCookie)
      ) {
        $.ajax({
      ...
      if (fileName.split(".").pop() == "svg") {
        const SVGContent = $.get({
          ...
          success(data, status, { responseText }) {
            const iconWrapper = self.options.fadeIn
                ? $(
  ...
  class PluginParallax {
    ...
    build() {
      ...
      if (
        ...
        const $scrollableWrapper = self.options.wrapper.find(
        ...
        if ($scrollableWrapper.get(0)) {
          ...
          const cssValueUnit = self.options.cssValueUnit
...
((theme = {}, $) => {
  ...
  class PluginScrollSpy {
    ...
    build() {
      ...
      let sectionIDs = Object.keys(navItems).map(
        (key, index) => navItems[key].hash
      );
  ...
  class PluginSectionScroll {
    ...
    build() {
        ...
        $(window).on("scroll", () => {
          ...
          $(".section-scroll").each(function () {
            if ($(this).offset().top <= $(window).scrollTop() + 50) {
              const $currentSection2 = $(".section-wrapper")
                .eq(index)
                .find(".section-scroll");
      ...
      $(window).on(wheelEvent, ({ target, originalEvent }) => {
        ...
        if ($(window).width() < 992 && $("html").hasClass("touch")) {
          if (
            $(target).closest(".section-scroll-dots-navigation").get(0) ||
            $(target).closest(".header-body").get(0) ||
            $(target).closest(".owl-carousel").get(0)
          ) {
            return;
          }
        }
        ...
        const wheelDirection =
          originalEvent.wheelDelta == undefined
        ...
        const $currentSection = $(".section-wrapper")
          .eq(self.getCurrentIndex())
          .find(".section-scroll");
        const $nextSection = self.getNextSection(
          wheelDirection,
          touchDirection
        );
    ...
    events() {
      ...
      $(window).on("section.scroll.change.header.color", () => {
        const headerColor = $(".section-wrapper")
          .eq(self.getCurrentIndex())
          .find(".section-scroll")
...
((theme = {}, $) => {
  ...
  class PluginSticky {
    ...
    events() {
      ...
      const $window = $(window);
      const $logo = self.options.wrapper.find("img");
      const classToCheck = self.options.wrapper.hasClass(
...
((theme = {}, $) => {
  ...
  class PluginVideoBackground {
    ...
    build() {
      ...
      if ($playButton.get(0)) {
        const $playWrapper = self.options.wrapper.find(
            ...
            setTimeout(() => {
              self.$wrapper.find("input").on("focus", () => {
                ...
                $(document).mouseup(({ target }) => {
                  if (
                    !self.$wrapper.is(target) &&
                    self.$wrapper.has(target).length === 0
                  ) {
      ...
      events() {
            ...
            if ($window.width() > 991) {
              ...
              if (!$(this).closest("li").hasClass("open")) {
                ...
                $(document)
                  ...
                  .on("click.nav-click-to-open", ({ target }) => {
        ...
        $header.find(".header-nav-features-toggle").on("click", function (e) {
          ...
          if (
            ...
            $(document)
              ...
              .on("click.header-nav-features-toggle", ({ target }) => {
                if (
                  !$toggleParent.is(target) &&
                  $toggleParent.has(target).length === 0
                ) {
                  $(".header-nav-features-dropdown.show").removeClass("show");
                }
              });
        ...
        if ($("[data-hash]").length) {
          $("[data-hash]").on("mouseover", function () {
            ...
            if (!$this.data("__dataHashBinded")) {
              ...
              let offset = $this.is("[data-hash-offset]")
                ? $this.data("hash-offset")
                : 0;
              const delay = $this.is("[data-hash-delay]")
        ...
        if ($(".side-panel-toggle").length) {
          ...
          $(document).on("click", ({ target }) => {
            if (
              !$(target).closest(".side-panel-wrapper").length &&
              !$(target).hasClass("side-panel-toggle")
            ) {
              $(
  ...
  $.extend(theme, {
    StickyHeader: {
      ...
      build() {
        ...
        const initialHeaderTopHeight = self.options.wrapper
          .find(".header-top")
          .outerHeight();
        const initialHeaderContainerHeight = self.options.wrapper
